...
{
    public class ItemCountVM<T> : ModelDataVM<IdCountPair<T>>
    {
    ...
    public static class IdCountPairVM
    {
        ...
        ) where TViewModel : ModelDataVM<TId, TRef>
        {
            var model = new IdCountPair<TId>(referenceVM.Model);
    ...
    public class IdCountPairVM<TId, TModel, TViewModel> : NotifyPropertyChangedBase
        where TViewModel : ModelDataVM<TId, TModel>
    {
